...
class MenubarNavigation {
    constructor(domNode) {
        ...
        // --- Zamykanie menu po wyjechaniu kursorem z całej belki ---
        const desktopNavbar = document.querySelector('.pc_desktop_menu');

        // Timer na drobne opóźnienie, żeby menu nie znikało natychmiast
        this._hoverCloseTimer = null;

        if (desktopNavbar) {
            desktopNavbar.addEventListener('pointerleave', () => {
                clearTimeout(this._hoverCloseTimer);
                this._hoverCloseTimer = setTimeout(() => {
                    // Upewnij się, że kursor naprawdę opuścił obszar belki
                    if (!desktopNavbar.matches(':hover')) {
                        this.setMenubarDataExpanded('false');
                        this.closePopupAll();
                    }
                }, 180); // 180 ms opóźnienia – możesz dostosować
            });

            // Jeśli użytkownik wróci myszką na belkę, anuluj zamknięcie
            desktopNavbar.addEventListener('pointerenter', () => {
                clearTimeout(this._hoverCloseTimer);
            });
        }

        // const navbar = document.querySelector('.pc_desktop_menu');
        // navbar.addEventListener(
        //     'pointerleave',
        //     this.onBackgroundPointerleave.bind(this)
        // );
    ...
    initMenu(menu, depth) {
        ...
        for (var i = 0; i < menuitems.length; i++) {
            ...
            menuitem.addEventListener(
                'mouseover',
                this.onBackgroundPointerover.bind(this)
            );
    ...
    openPopup(menuId, menuitem) {
        ...
        if (popupMenu) {
            ...
            const isChildHook = popupMenu.querySelector(".hook-in-menu") ? true : false;

            const displayStyle = isChildHook ? "grid" : "flex";


            // Set CSS properties
            if (this.isPopup[menuId]) {
                popupMenu.parentNode.style.position = 'relative';
                popupMenu.style.display = displayStyle;
                ...
                popupMenu.style.top = desktopNavBar.offsetHeight + 8 + 'px';
                popupMenu.style.zIndex = 100;
            }
